...
status_event *status_events_select(int id, int *state) {
  ...
  if (NULL == (db = fopen("../../materials/master_status_events.db", "r+b"))) {
    *state = ERROR;
    ...
    selected = select(db, id, sizeof(status_event));
    fclose(db);
  }
  return selected;
}
...
int status_events_delete(int id, int *state) {
  ...
  if (NULL == (db = fopen("../../materials/master_status_events.db", "r+b"))) {
    *state = ERROR;
    ...
    result = delete(db, id, sizeof(status_event));
    fclose(db);
  }
  return result;
}
...
int status_events_insert(status_event *new_status_event, int *state) {
  ...
  if (NULL == (db = fopen("../../materials/master_status_events.db", "r+b"))) {
    *state = ERROR;
    ...
    result = insert(db, new_status_event, sizeof(status_event));
    fclose(db);
  }
  return result;
}
...
int status_events_update(int id, status_event *updated_status_event, int *state) {
  ...
  if (NULL == (db = fopen("../../materials/master_status_events.db", "r+b"))) {
    *state = ERROR;
    ...
    result = update(db, id, updated_status_event, sizeof(status_event));
    fclose(db);
  }
  return result;
}
    ...
    *state = ERROR;
    ...
    if (RUNNING == *state) {
      if (0 == result) {
        ...
        printf("The status event was successfully updated!\n");
      }
    }
  }
}
