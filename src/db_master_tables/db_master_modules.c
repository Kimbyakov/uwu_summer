...
ai_module *ai_modules_select(int id, int *state) {
  ...
  if (NULL == (db = fopen("../materials/master_modules.db", "r+b"))) {
    *state = ERROR;
    ...
    selected = select(db, id, sizeof(ai_module));
    fclose(db);
  }
  return selected;
}
...
int ai_modules_delete(int id, int *state) {
  ...
  if (NULL == (db = fopen("../materials/master_modules.db", "r+b"))) {
    *state = ERROR;
    ...
    result = delete(db, id, sizeof(ai_module));
    fclose(db);
  }
  return result;
}
...
int ai_modules_insert(ai_module *new_ai_module, int *state) {
  ...
  if (NULL == (db = fopen("../materials/master_modules.db", "r+b"))) {
    *state = ERROR;
    ...
    result = insert(db, new_ai_module, sizeof(ai_module));
    fclose(db);
  }
  return result;
}
...
int ai_modules_update(int id, ai_module *updated_ai_module, int *state) {
  ...
  if (NULL == (db = fopen("../materials/master_modules.db", "r+b"))) {
    *state = ERROR;
    ...
    result = update(db, id, updated_ai_module, sizeof(ai_module));
    fclose(db);
  }
  return result;
}
    ...
    *state = ERROR;
    ...
    if (RUNNING == *state) {
      if (0 == result) {
        ...
        printf("The module was successfully updated!\n");
      }
    }
  }
}
